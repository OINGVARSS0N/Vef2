...
describe('Category Functions', () => {
  ...
  describe('updateCategory', () => {
    it('should update an existing category', async () => {
      ...
      const updatedTitle = 'Updated Category';
      const mockUpdatedCategory = { id: 1, slug: 'updated-category', title: updatedTitle };
      prisma.categories.update.mockResolvedValue(mockUpdatedCategory);

      const result = await updateCategory(slug, updatedTitle);
      expect(result).toEqual(mockUpdatedCategory);
      expect(prisma.categories.update).toHaveBeenCalledWith({
        where: { slug },
        data: {
          title: updatedTitle,
          slug: updatedTitle.toLowerCase().replace(' ', '-'),
        },
      });
    });
  });
